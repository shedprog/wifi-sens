...
def serial_handle(queue_read, queue_write, port):
    ...
    for data_valid_index, data_valid in data_valid_list.iterrows():
        ...
        data_valid_list.at[data_valid_index, 'file_fd'] = file_fd
        data_valid_list.at[data_valid_index, 'file_writer'] = file_writer
    ...
    while True:
        ...
        for _, data_valid in data_valid_list.iterrows():
            ...
            if index >= 0:
                ...
                if len(data) == len(data_valid['columns_names']):
                    ...
                    break
                elif data_valid['type'] == 'CSI_DATA':
                    print(f"CSI_DATA column mismatch: expected {len(data_valid['columns_names'])}, got {len(data)}")
                    log_data_writer.writelines(
                        f"CSI_DATA column mismatch: expected {len(data_valid['columns_names'])}, got {len(data)}\n{strings}\n"
                    )
